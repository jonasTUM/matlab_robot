%   This class is based on the MATLAB provided StateSpaceExample.m file.
%   The calculation for distance() is implemented customly.
...
classdef CustomConfigurationSpace < nav.StateSpace & ...
    ...
    methods
        ...
        %
        function obj = CustomConfigurationSpace(qMin, qMax)
            
            spaceName = "CustomConfigurationSpace";
            numStateVariables = length(qMin);
            % For each state variable define the lower and upper valid
            % limit (one [min,max] limit per row)
            stateBounds = zeros(numStateVariables, 2);
            for i = 1: numStateVariables
                stateBounds(i,1) = qMin(i);
                stateBounds(i,2) = qMax(i);
            end
        ...
        %
        function dist = distance(~, state2, state1)
            numOfStates = size(state2,2);
            numOfSamples = size(state2,1);
            D = zeros(numOfSamples,numOfStates);
            dist = zeros(numOfSamples,1);
            for ii = 1:numOfSamples
                D(ii,:) = state2(ii,:) - state1;
                dist(ii,1) = norm(D(ii,1:numOfStates));
            end
        end  
    end
end
